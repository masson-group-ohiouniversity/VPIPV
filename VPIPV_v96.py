...
def load_excel_inputs(xlsx_bytes: bytes):
    ...
    # Experimental data (rows 24+):
    # New template: col A = V added (mL), col B = %S0, col C = %S1, col D = %S2, col E = %S3
    # Old template: col A = V added (mL), col B = %S3 only
    # Auto-detect by checking header row 23
    hB = _as_str(ws["B23"].value).lower()
    hC = _as_str(ws["C23"].value).lower()
    is_multi = ("s0" in hB) or ("s1" in hC)

    v_added = []
    pct_s0, pct_s1, pct_s2, pct_s3 = [], [], [], []
    r = 24
    while True:
        va = ws[f"A{r}"].value
        if is_multi:
            b, c, d, e = ws[f"B{r}"].value, ws[f"C{r}"].value, ws[f"D{r}"].value, ws[f"E{r}"].value
            if va is None and b is None and c is None and d is None and e is None:
                break
            va_f = _to_float_or_none(va)
            if va_f is not None:
                v_added.append(float(va_f))
                pct_s0.append(_to_float_or_none(b) if _to_float_or_none(b) is not None else float("nan"))
                pct_s1.append(_to_float_or_none(c) if _to_float_or_none(c) is not None else float("nan"))
                pct_s2.append(_to_float_or_none(d) if _to_float_or_none(d) is not None else float("nan"))
                pct_s3.append(_to_float_or_none(e) if _to_float_or_none(e) is not None else float("nan"))
        else:
            ys = ws[f"B{r}"].value
            ...
            if va_f is not None:
                v_added.append(float(va_f))
                pct_s0.append(float("nan"))
                pct_s1.append(float("nan"))
                pct_s2.append(float("nan"))
                pct_s3.append(float(ys_f) if ys_f is not None else float("nan"))
        r += 1
        if r > 5000:
            break
    ...
    return {
        ...
        "pctS0": np.array(pct_s0, dtype=float),
        "pctS1": np.array(pct_s1, dtype=float),
        "pctS2": np.array(pct_s2, dtype=float),
...
def pctS_from_params_at_equivs(p: dict, equivs: np.ndarray) -> dict:
    """Return predicted %S0..%S3 at the given equiv points."""
    p_local = dict(p)
    p_local["xs_custom"] = np.array(equivs, dtype=float)
    p_local["yMode"] = "pct"
    xs, y, S0, S1, S2, S3, _, _, _, _ = _compute_curve_once(p_local)
    total = S0 + S1 + S2 + S3
    return {
        "S0": np.where(total > 0, 100.0 * S0 / total, 0.0),
        "S1": np.where(total > 0, 100.0 * S1 / total, 0.0),
        "S2": np.where(total > 0, 100.0 * S2 / total, 0.0),
        "S3": np.where(total > 0, 100.0 * S3 / total, 0.0),
    }

def fit_logKs_to_pctS_multi(p: dict, fit_keys: list, equivs: np.ndarray,
                              exp_pct: dict, target: str):
    """Fit logK parameters to %S data.
    target: "S0", "S1", "S2", "S3", or "ALL" (concatenated residuals).
    exp_pct: dict with keys "S0".."S3", each a np.ndarray (nan = missing point).
    """
    fit_keys = list(fit_keys)
    x0 = np.array([float(p[k]) for k in fit_keys], dtype=float)
    ...
    def residuals(x):
        pct_pred = pctS_from_params_at_equivs(unpack(x), equivs)
        if target == "ALL":
            parts = []
            for s in ("S0", "S1", "S2", "S3"):
                ...
                mask = np.isfinite(obs) & np.isfinite(pct_pred[s])
                if np.any(mask):
                    parts.append(pct_pred[s][mask] - obs[mask])
            return np.concatenate(parts) if parts else np.array([1e6])
        else:
            obs = np.array(exp_pct.get(target, []), dtype=float)
            mask = np.isfinite(obs) & np.isfinite(pct_pred[target])
            if not np.any(mask):
                return np.array([1e6])
            return pct_pred[target][mask] - obs[mask]

    lo = np.full_like(x0, -10.0)
    hi = np.full_like(x0,  20.0)
    res = least_squares(residuals, x0, bounds=(lo, hi), method="trf")
...
def _compute_curve_once(p: dict):
    ...
    for i, eq in enumerate(xs):
        ...
        warn[i] = (not np.isfinite(resid_norm[i])) or (resid_norm[i] > 1e-10)  # residual-only flag

        c = coeffs_G(H, M, p)
        Gfree = Gtot / sum(c.values())

        S0_i = (c.get("G", 0.0) + c.get("GM", 0.0) + c.get("GM2", 0.0)) * Gfree
...
with st.sidebar:
    # Use a changing key so re-uploading the *same* filename still triggers a widget change in the browser.
    # (Some browsers won't fire a change event if you pick the same file again unless the input is cleared.)
    if "_uploader_nonce" not in st.session_state:
        st.session_state["_uploader_nonce"] = 0

    # Some browsers (notably Firefox) may not always trigger a new upload event when the *same* filename
    # is selected again. This explicit clear makes re-uploading the same file reliable.
    ...
    uploaded_xlsx = st.file_uploader(
        "Open File",
        type=["xlsx"],
        key=f"open_file_{st.session_state['_uploader_nonce']}",
    )
    ...
    if uploaded_xlsx is not None:
        try:
            ...
            st.session_state["excel_pctS0"] = excel_inputs.get("pctS0", np.full(len(excel_inputs["v_added_mL"]), float("nan")))
            st.session_state["excel_pctS1"] = excel_inputs.get("pctS1", np.full(len(excel_inputs["v_added_mL"]), float("nan")))
            st.session_state["excel_pctS2"] = excel_inputs.get("pctS2", np.full(len(excel_inputs["v_added_mL"]), float("nan")))
            st.session_state["excel_pctS3"] = excel_inputs["pctS3"]
            if (st.session_state.get("excel_digest") != excel_digest) and (st.session_state.get("_pending_excel_seed", {}).get("digest") != excel_digest):
                # New Excel file detected. Streamlit widgets can keep "sticky" values from the previous upload.
                # To ensure the left sidebar fully re-seeds from the new Excel file, we apply defaults in
                # a two-step rerun:
                #   1) Clear widget keys + store pending seed, then rerun
                #   2) On next run, apply pending seed BEFORE widgets are created, then rerun once more
                _excel_keys = ["logKH3M","logKH3M2","logKH1","logKH2","logKH3","logKHM","logKHM2","logKM0","logKM20","logKM1","logKM21","logKM2","logKM22","logKQ","logKQM","logKQHM"]
                _reset_keys = [
                    "G0_mM","H0_mM","Q0_mM","V0_mL","Mtitrant_mM","xMaxBox",
                    "enable_Q","enable_Q_choice",
                    "allow_KH3","allow_KH3_choice",
                    "allow_HM","allow_HM_choice",
                    "excel_fit_flags","do_fit",
                ] + _excel_keys + [f"fit_{k}" for k in _excel_keys]
                for _rk in _reset_keys:
                    st.session_state.pop(_rk, None)

                st.session_state["_pending_excel_seed"] = {"digest": excel_digest, "inputs": excel_inputs}
                st.session_state["_apply_excel_seed"] = True
                st.rerun()

            # Apply pending seed BEFORE widgets are created so the sidebar updates correctly
            if st.session_state.get("_apply_excel_seed") and st.session_state.get("_pending_excel_seed"):
                _seed = st.session_state["_pending_excel_seed"]
                _seed_inputs = _seed.get("inputs", {})
                _seed_digest = _seed.get("digest")

                _excel_keys = ["logKH3M","logKH3M2","logKH1","logKH2","logKH3","logKHM","logKHM2","logKM0","logKM20","logKM1","logKM21","logKM2","logKM22","logKQ","logKQM","logKQHM"]

                st.session_state["excel_fit_flags"] = _seed_inputs.get("fit_flags", [])
                for _k in _excel_keys:
                    st.session_state[f"fit_{_k}"] = (_k in st.session_state.get("excel_fit_flags", []))

                if _seed_inputs.get("G0_mM") is not None:
                    st.session_state["G0_mM"] = float(_seed_inputs["G0_mM"])
                if _seed_inputs.get("H0_mM") is not None:
                    st.session_state["H0_mM"] = float(_seed_inputs["H0_mM"])
                if _seed_inputs.get("V0_mL") is not None:
                    st.session_state["V0_mL"] = float(_seed_inputs["V0_mL"])
                if _seed_inputs.get("Mtitrant_mM") is not None:
                    st.session_state["Mtitrant_mM"] = float(_seed_inputs["Mtitrant_mM"])
                if _seed_inputs.get("Q0_mM") is not None:
                    st.session_state["Q0_mM"] = float(_seed_inputs["Q0_mM"])
                if _seed_inputs.get("xMaxBox") is not None:
                    st.session_state["xMaxBox"] = float(_seed_inputs["xMaxBox"])

                st.session_state["enable_Q"] = bool(_seed_inputs.get("enable_Q", False))
                st.session_state["enable_Q_choice"] = "Yes" if st.session_state["enable_Q"] else "No"

                st.session_state["allow_KH3"] = bool(_seed_inputs.get("allow_KH3", True))
                st.session_state["allow_KH3_choice"] = "Yes" if st.session_state["allow_KH3"] else "No (KH3=0)"

                st.session_state["allow_HM"] = bool(_seed_inputs.get("allow_HM", True))
                st.session_state["allow_HM_choice"] = "Yes" if st.session_state["allow_HM"] else "No (KHM=0)"

                for k, v in _seed_inputs.get("const_vals", {}).items():
                    st.session_state[k] = float(v)

                st.session_state["excel_digest"] = _seed_digest
                # Clear the file input so the user can select the same file name again and still force a refresh.
                st.session_state["_uploader_nonce"] = int(st.session_state.get("_uploader_nonce", 0)) + 1
                st.session_state["_apply_excel_seed"] = False
                st.session_state.pop("_pending_excel_seed", None)
                st.rerun()
        except Exception as e:
            st.error(f"Could not parse Excel file: {e}")
    # --- Fit target + Run Fit button (shown whenever Excel is loaded) ---
    if st.session_state.get("excel_loaded", False):
        ...
        _avail = {
            "S0": _has_data(st.session_state.get("excel_pctS0")),
            "S1": _has_data(st.session_state.get("excel_pctS1")),
            "S2": _has_data(st.session_state.get("excel_pctS2")),
            "S3": _has_data(st.session_state.get("excel_pctS3")),
        }
        _all_avail = all(_avail.values())
        _target_opts = [s for s in ("S0","S1","S2","S3") if _avail[s]] + (["ALL"] if _all_avail else [])
        if not _target_opts:
            _target_opts = ["S3"]
        if st.session_state.get("fit_target") not in _target_opts:
            st.session_state["fit_target"] = _target_opts[-1]  # default to last (ALL if available, else S3)
        st.session_state["fit_target"] = st.selectbox(
            "Fit target", _target_opts,
            index=_target_opts.index(st.session_state["fit_target"]),
            key="fit_target_select"
        )
        _all_logk_keys = ["logKH3M","logKH3M2","logKH1","logKH2","logKH3","logKHM","logKHM2",
                          "logKM0","logKM20","logKM1","logKM21","logKM2","logKM22","logKQ","logKQM","logKQHM"]
        _any_fit_checked = any(st.session_state.get(f"fit_{k}", False) for k in _all_logk_keys)
        if st.button("▶ Run Fit", disabled=not _any_fit_checked,
                     help="Tick at least one 'fit' checkbox next to a constant below, then click here.",
...
exp_equiv = None
exp_pct = {"S0": None, "S1": None, "S2": None, "S3": None}  # %S arrays at exp_equiv points
exp_mM  = {"S0": None, "S1": None, "S2": None, "S3": None}  # same in mM
if st.session_state.get("excel_loaded", False):
    v_added = st.session_state.get("excel_v_added_mL", None)
    if v_added is not None and len(v_added) > 0:
        exp_equiv = equiv_from_vadded(v_added, params["G0_mM"], params["V0_mL"], params["Mtitrant_mM"])
        Vtot = float(params["V0_mL"]) + np.array(v_added, dtype=float)
        nG0  = float(params["G0_mM"]) * float(params["V0_mL"])
        for _s, _sk in [("S0","excel_pctS0"),("S1","excel_pctS1"),("S2","excel_pctS2"),("S3","excel_pctS3")]:
            _arr = st.session_state.get(_sk)
            if _arr is not None:
                _arr = np.array(_arr, dtype=float)
                if _arr.size > 0 and np.any(np.isfinite(_arr)):
                    exp_pct[_s] = _arr
                    exp_mM[_s]  = (_arr / 100.0) * nG0 / Vtot

# --- Optional fitting ---
if st.session_state.get("do_fit", False):
    _all_logk_keys = ["logKH3M","logKH3M2","logKH1","logKH2","logKH3","logKHM","logKHM2",
                      "logKM0","logKM20","logKM1","logKM21","logKM2","logKM22","logKQ","logKQM","logKQHM"]
    fit_keys = [k for k in _all_logk_keys if st.session_state.get(f"fit_{k}", False)]
    fit_target = st.session_state.get("fit_target", "S3")
    # Validate
    _target_ok = False
    if exp_equiv is not None:
        if fit_target == "ALL":
            _target_ok = any(exp_pct[s] is not None for s in ("S0","S1","S2","S3"))
        else:
            _target_ok = exp_pct.get(fit_target) is not None
    if not _target_ok or len(fit_keys) == 0:
        st.sidebar.error("Fit requested, but no experimental data for the selected target, or no 'fit' checkboxes ticked.")
        st.session_state["do_fit"] = False
    else:
        p_fit, res = fit_logKs_to_pctS_multi(params, fit_keys, exp_equiv, exp_pct, fit_target)
...
with col1:
    ...
    # Fixed colors: S0=blue, S1=red, S2=green, S3=violet
    _col = {"S0": "#636EFA", "S1": "#EF553B", "S2": "#00CC96", "S3": "#AB63FA"}

    if params["yMode"] == "S3mM":
        fig.add_trace(go.Scatter(x=xs, y=S0, mode="lines",
                                 name="S0 (mM)", line=dict(dash="dot",      color=_col["S0"])))
        fig.add_trace(go.Scatter(x=xs, y=S1, mode="lines",
                                 name="S1 (mM)", line=dict(dash="dash",     color=_col["S1"])))
        fig.add_trace(go.Scatter(x=xs, y=S2, mode="lines",
                                 name="S2 (mM)", line=dict(dash="longdash", color=_col["S2"])))
        fig.add_trace(go.Scatter(x=xs, y=S3, mode="lines",
                                 name="S3 (mM)", line=dict(width=3,         color=_col["S3"])))
    else:
        fig.add_trace(go.Scatter(x=xs, y=pct_S0, mode="lines",
                                 name="S0 (%)", line=dict(dash="dot",      color=_col["S0"])))
        fig.add_trace(go.Scatter(x=xs, y=pct_S1, mode="lines",
                                 name="S1 (%)", line=dict(dash="dash",     color=_col["S1"])))
        fig.add_trace(go.Scatter(x=xs, y=pct_S2, mode="lines",
                                 name="S2 (%)", line=dict(dash="longdash", color=_col["S2"])))
        fig.add_trace(go.Scatter(x=xs, y=pct_S3, mode="lines",
                                 name="S3 (%)", line=dict(width=3,         color=_col["S3"])))
    ...
    # Experimental points — color-matched to calculated traces, open circle markers
    if exp_equiv is not None:
        _exp_y = exp_mM if params["yMode"] == "S3mM" else exp_pct
        for _s in ("S0","S1","S2","S3"):
            _arr = _exp_y.get(_s)
            if _arr is None:
                continue
            _arr = np.array(_arr, dtype=float)
            ...
            if not np.any(_mask):
                continue
            fig.add_trace(go.Scatter(
                x=np.array(exp_equiv, dtype=float)[_mask],
                y=_arr[_mask],
                mode="markers",
                name=f"Excel {_s}",
                marker=dict(size=6, symbol="circle-open", color=_col[_s], line=dict(width=1, color=_col[_s]))
            ))
...
with col2:
    ...
    if fit_report is not None:
        ...
        st.write(f"**Target:** {fit_report.get('target', 'S3')}")
        st.write(f"**Success:** {fit_report.get('success', False)}")
        st.write(f"**RMSE (%):** {fit_report.get('rmse', float('nan')):.4g}")
